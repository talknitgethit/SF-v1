"""Exception types raised deliberately by SentinelForge.

Every error SentinelForge raises on purpose inherits from
:class:`SentinelForgeError`. That gives the CLI a single, narrow thing to catch:
...
bug in SentinelForge, is left to propagate as a traceback.
...
class SentinelForgeError(Exception):
    """Base class for all errors SentinelForge raises on purpose."""


class EvidenceError(SentinelForgeError):
...
class ReportError(SentinelForgeError):
